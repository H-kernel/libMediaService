...
int h264_mp4toannexb_oneframe(const struct mpeg4_avc_t* avc, const void* data, int bytes, void* out, int size,int* dealsize,char** pLeft,int* pLeftsize)
{
	int i = 0;
	int n = 0;
	uint8_t sps_pps_flag;
	...
	*pLeftsize = 0;
	...
	if (dst + n + sizeof(h264_start_code) > (uint8_t*)out + size)
	{
        int need = (((uint8_t*)out + size) - dst) - sizeof(h264_start_code);
		memcpy(dst, h264_start_code, sizeof(h264_start_code));
		memcpy(dst + sizeof(h264_start_code), src + avc->nalu,need);
		dst += sizeof(h264_start_code) + need;
	    *dealsize  = avc->nalu + n;

		*pLeftsize = n - need;
		*pLeft     = src + avc->nalu + need;

		return dst - (uint8_t*)out;
	}
	//memcpy(dst, src + avc->nalu, n);
	memcpy(dst, h264_start_code, sizeof(h264_start_code));
	memcpy(dst + sizeof(h264_start_code), src + avc->nalu, n);
	dst += sizeof(h264_start_code) + n;
	*dealsize  = avc->nalu + n;
