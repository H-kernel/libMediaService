...
static int h264_write(struct mov_h264_write_t* ctx, const void* data, int bytes)
{
    int vcl = 0;
    int update = 0;
...
{
	struct mov_h264_write_t* ctx = (struct mov_h264_write_t*)param;
	assert(ctx->ptr < nalu);
